...
# Bot information
API_ID = environ.get('API_ID', '25993194')  #api id of your telegram id
if len(API_ID) == 0:
    print('Error - API_ID is missing, exiting now')
    exit()
else:
    API_ID = int(API_ID)
API_HASH = environ.get('API_HASH', '63c16a1e4bec73cc56edbd4e5dbd7a4a') #api hash of your telegram id
if len(API_HASH) == 0:
...
# Bot Admins
ADMINS = environ.get('ADMINS', '1278741735') #apni tg id daalo
if len(ADMINS) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1002096102510').split()]
if len(INDEX_CHANNELS) == 0:
    print('Info - INDEX_CHANNELS is empty')
AUTH_CHANNEL = [int(auth_channels) for auth_channels in environ.get('AUTH_CHANNEL', '-1002107479581').split()]
if len(AUTH_CHANNEL) == 0:
    print('Info - AUTH_CHANNEL is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1002048542710') #bot log channel -1005293546253
...
# support group
SUPPORT_GROUP = environ.get('SUPPORT_GROUP', '-1002139377076') #support group id ex:  -1002936246860
...
# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://dasaripardhasaradhi141:X5SstfxEbJORxOTh@cluster0.fvgzzwq.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0") #mongo db url
...
BIN_CHANNEL = environ.get("BIN_CHANNEL", "-1002048542710") #if is_stream = true then add a channel id ex: -10026393639
if len(BIN_CHANNEL) == 0:
    print('Error - BIN_CHANNEL is missing, exiting now')
    exit()
else:
    BIN_CHANNEL = int(BIN_CHANNEL)
URL = environ.get("URL", "https://dps-rs-auto-filter-bot-26f38ebf6505.herokuapp.com/") #if heroku then paste the app link here ex: https://heroku......./
if len(URL) == 0:
